...
public class DagrunLagerTing extends DatabaseAPI{
	...
	public ArrayList<Appointment> findAppointments(User user) throws SQLException{
	...
	public ArrayList<User> getUserList(int appointmentID) throws SQLException{
		ArrayList<User> liste = new ArrayList<User>();
		
		String st = "SELECT * FROM deltager,bruker WHERE avteleID LIKE "+appointmentID+" AND bruker.brukernavn = deltager.brukernavn";
		ResultSet rs = conn.createStatement().executeQuery(st);	
		while(rs.next()){
